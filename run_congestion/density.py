...
def render_cli_block(roll: SegmentRollup) -> str:
    ...
    z = roll.zones_km
    parts = []
    title = f"🔍 Checking {seg.event_a}" + (f" vs {seg.event_b}" if seg.event_b else "") + \            f" from {seg.km_from:.2f}km–{seg.km_to:.2f}km..."
    parts.append(title)
    parts.append(f"📝 Segment: width={seg.width_m} m, direction={seg.direction}")
    parts.append(f"👥 Peak concurrent: {roll.peak['combined']} ({roll.peak}) @ {roll.peak_km:.2f} km")
    parts.append(f"📈 Density (peak step): {roll.peak_step_areal_m2} /m² (Linear ≈ {roll.peak_step_linear_m} /m)")
    parts.append(f"📉 Density (segment-average @ peak): {roll.segment_avg_at_peak_areal_m2} /m² (Linear ≈ {roll.segment_avg_at_peak_linear_m} /m)")
    parts.append(f"🚦 Zones (km): G={z['green']}, A={z['amber']}, R={z['red']}, D={z['dark_red']}")
    parts.append(f"🧮 Index: {roll.index_0_10}/10")
    return "\n".join(parts)
